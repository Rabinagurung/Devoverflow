...
export async function getUserQuestions(
  ...
  try {
    ...
    const isNext = totalAnswers > questions.length + skip;
...
export async function getUserTopTags(params: GetUserTagsParams): Promise<
  ActionResponse<{
    tags: { _id: string; name: string; questionsCount: number }[];
  }>
> {
  ...
  try {
    const pipeline: PipelineStage[] = [
      { $match: { author: new Types.ObjectId(userId) } }, // Find user's questionsCount
      { $unwind: "$tags" }, // Flatten tags array
      { $group: { _id: "$tags", questionsCount: { $sum: 1 } } }, // questionsCount occurrences
      {
        $lookup: {
          from: "tags",
          localField: "_id",
      ...
      { $unwind: "$tagInfo" },
      { $sort: { questionsCount: -1 } }, // Sort by most used
      { $limit: 10 }, // Get top 10
      {
        $project: {
          _id: "$tagInfo._id",
